...
class TensorboardPyTorch:
    ...
    def log_reconstructions_visualize(self, model, loader, epoch, cutoff=8):
        import torchvision
        # Reconstruct images
        model.eval()
        x_true, y_true = next(iter(loader))
        with torch.no_grad():
            x_reconst = model(x_true.to(self.device))[0].detach().cpu()

        # Plotting
        imgs = torch.stack([x_true[:cutoff], x_reconst[:cutoff]], dim=1).flatten(0, 1)
        grid = torchvision.utils.make_grid(imgs, nrow=cutoff)
        # grid = grid.permute(1, 2, 0)
        self.writer.add_image(f'Reconstruction/Epoch--{epoch}', grid, global_step=epoch)
        self.flush()